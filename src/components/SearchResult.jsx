...
const SearchResult = () => {
  ...
  const fetchResult=()=>{
    ...
    fetchDataFromApi(payload).then(res=>{setResult(res)
    console.log(res)})
  }
  if(!result)return;
  ...
  return (
    <>
        ...
        <main>
          ...
          {
            !imageSearch ? (items.map((item,index)=>(<SearchItemBox key={index} data={item}></SearchItemBox>))):(
              <div className='SerachImageAllItem'>
                {items.map((item,index)=>(<SerachImageItem key={index} data={item}></SerachImageItem>))}
              </div>
            )
          }
        </main>

        ...
        <div className='tabAndLogo'>
            {
                queries.previousPage &&(
                    <div className='arrowWithDiv' onClick={()=>pageClickHandler(queries.previousPage[0].startIndex)}>
                        <FiChevronLeft size={20}></FiChevronLeft>
                        <div>Pre</div>
                    </div>
                )
            }
            <img className='paginationImage' src={paginationImage} alt="pagination-logo" />
            {
                queries.nextPage  &&(
                    <div  className='arrowWithDiv' onClick={()=>pageClickHandler(queries.nextPage[0].startIndex)}>
        ...
        <div className='spanPageNumber'>
            {
                pagination.map((data)=>(
                    ...
                    className={`pageNumber ${page===data.startIndex? "activePage":null}`}
                    >{data.page}</span>
                ))
            }
        </div>
